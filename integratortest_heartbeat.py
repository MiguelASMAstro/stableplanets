import numpy as np
import rebound
import matplotlib.pyplot as plt
import time
import sys

t_end = 1e6*2*np.pi
p2p1 = 1.8

sim = rebound.Simulation()
sim.add(m=1)
sim.add(m=0.00005149, P=2*np.pi,e=0.03,inc=0.01,f=np.pi)
sim.add(m=0.00005149, P=p2p1*2*np.pi,e=0.03,inc=0.01,f=0.75*np.pi)

a1 = sim.particles[1].a + 1.5*0.00005149**(2/7)
a2 = sim.particles[2].a - 1.5*0.00005149**(2/7)

sim.integrator = "whfast"
sim.dt = 0.005*2*np.pi
sim.ri_whfast.safe_mode = False

a_s = np.linspace(a1,a2,num=10)
a_initial = a_s[4]
f_initial = 0.25*np.pi+0.00392699081*np.random.uniform(low=-1,high=1)

sim.add(a=a_initial,f=f_initial)
sim.N_active = 3

sim.move_to_com()

sim.exit_max_distance = 10.

start = time.perf_counter()

def heartbeat(sim_pointer):
    sim = sim_pointer.contents
    o = sim.particles[3].calculate_orbit()
    peri = o.a*(1-o.e)
    apo = o.a*(1+o.e)
    a1 = sim.particles[1].calculate_orbit().a
    a2 = sim.particles[2].calculate_orbit().a
    if peri <= a1:
        stop = time.perf_counter()
        print(a_initial,f_initial,sim.t/2/np.pi,1,stop-start)
        sim.stop()
        return
    elif apo >= a2:
        stop = time.perf_counter()
        print(a_initial,f_initial,sim.t/2/np.pi,2,stop-start)
        sim.stop()
        return
    else:
        return

sim.heartbeat = heartbeat

try:
    sim.integrate(t_end)
    stop = time.perf_counter()
    print(a_initial,f_initial,sim.t/2/np.pi,0,stop-start)
except rebound.Escape as esc:
    stop = time.perf_counter()
    print(a_initial,f_initial,sim.t/2/np.pi,-1,stop-start)
# except rebound.Collision as col:
#     stop = time.perf_counter()
#     for i,p in enumerate(sim.particles[1:3]):
#         if p.lastcollision == sim.t:
#             print(a_initial,sim.t/2/np.pi,i+1,stop-start)

